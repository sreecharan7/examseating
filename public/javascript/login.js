...
async function formSubmission(event){
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4) {
            ...
            if(xhr.status==200){
                var box=`<div class="alert alert-success" role="alert">${response["redirect"]}. redirecting to page...</div>`
                wholePageMessageShower.innerHTML=box;
                setTimeout(function(){window.location.replace(`${response["redirect"]}`);},2000)
            }
            else{
